...
> 同行相对评分（`PEER_BENCHMARKS`）/ **行业细分基准（`INDUSTRY_SEGMENT_BENCHMARKS` ~25 细分，
> 解析顺序 细分>生意类型>generic；金融拆银行/保险/券商净利率口径）** /
> **真实同行中位数（5.1：`moat/peer_benchmarks.py`，AkShare 行业成分股财务中位，
> `peer_medians` 覆盖静态基准，`real_peer_medians` 开关，失败/无网回退静态表）** /
> 来源识别（无形资产·成本规模代理 + 研发费用率）/ 侵蚀信号 + `cycle_notes`（周期 ROE 波动不入侵蚀）/
> LLM 回填 + 宽度冲突处理 + `competition_evidence` 升级门槛与内容校验（5.10）/
> 参考池过滤市场情绪新闻（5.11）/ 周期行业 ROE·利润率·杠杆近 8 年跨周期均值（5.6/5.7）/
> 宽度合成门槛可配置（5.5）/ M1 依赖显式化（5.8）/ 「船舶·造船」周期关键词 / M9 契约闭环 /
> **档位相关性验证（5.9 横截面部分：`scripts/validate_moat_tiers.py`，真实数据
> 评分 vs 长期 ROE 秩相关 0.709）+ 光伏周期误判修复（拆出 solar 细分，隆基「无→窄」）** /
> provider 慢网加固（5.14：单次超时 + 总预算 + 并行拉取，防挂死主流程）/
> **M9 侵蚀风险 severity 细化（5.13：M5 回填 handoff.moat_trend，durability=low + trend=eroding → critical）**。
...
| 5.9 | **护城河档位前向回测** | 横截面验证已做（`scripts/validate_moat_tiers.py`，评分 vs 长期 ROE 秩相关 0.709）；「宽中窄是否预示 3–5 年后 ROE/盈利仍领先」的前向回测仍待做 | 用 `backtest/` 引擎做 PIT 前向检验：期初 M5 档位 → 期末 ROE/盈利持续性 | P2 |
| 5.14 | **同行中位 provider 打磨** | `moat/peer_benchmarks.py` 已实现，仍有口径细节：成分股样本上限 15 家、东财板块 vs 申万行业口径、最新年报中位 vs 多年度中位、财务指标拉取并发/缓存策略 | 按真实数据校验后调参：样本扩大（并发拉取）、按申万二级细分定板块、周期行业用同行跨周期中位 | P2 |
...
| 5.13 | **M9 侵蚀风险 severity 进一步细分** | 已实现 `durability=low + trend=eroding → critical`；可选继续：规则层「利润率压缩+杠杆抬升」双信号直接触发 critical、或 trend=widening 下调 severity |
