...
- **12.1 分行业口径**：同 M4，M2 按 M1 的 business_type / financial_subtype 路由
  （config/financial_routing.yaml 唯一事实来源，覆盖六个生意类型 + bank/broker/insurance 细类）。
  - 金融（银行/保险/券商）：现金流比仅用年报且阈值放宽（净利润主要由投资端决定、
    OCF/NP<0.8 不代表盈利质量差；银行直接 skip 按中性计），负债率 90%+ 视为行业常态不扣分；
  - 周期：底部 OCF/NP 天然偏低，阈值放宽到 0.4（仅年报），但杠杆仍是风险不放松；
  - 成长：再投资阶段 OCF/NP 可偏低，阈值放宽到 0.6（仅年报）；
  - 资产型/高分红/消费垄断：现金流是核心验证，保持通用硬阈值。
  目的：避免把金融/周期等行业误判成"财务质量极差"触发一票否决。
...
- **实现状态**：近 10 年窗口 + 分位去异常、按生意类型选主指标（周期/银行看 PB）、
  换手率情绪叠加（过热降分/过冷加分，只调置信度不改变价格位置）已落地（2026-08-07）；
  北向资金、两融余额、新发基金热度、舆论热度仍待接入。
