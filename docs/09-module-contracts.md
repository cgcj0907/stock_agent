...
| M6 | `dividend_years / payout_latest / note` + `llm_qualitative` | 更像分红代理模块，治理判断未上升为主输出 |
...
- 依赖：M2、M3、M5、M6、M7、M8（**修 YAML 缺失**）
- `risk_items[]`（统一风险项对象）：
  ```python
...
- `core_facts`：
  ...
  - `position`：建议仓位 0–1（veto 时强制 0）
  - `dimension_scores`（现有 dimensions）
  - `total`
- `qualitative`：`decision_reasons[]`（为什么得到该结论，规则解释 + 可选 LLM）
- `handoff`：
...
### M11 监控（monitor）
- 依赖：M10（读全部已跑模块的 handoff）
- `rules[]`（每条规则统一 schema，基于现有 `MonitorRule` 扩展）：
  ```python
  {"rule_type": "price_buy|price_sell|valuation_sell|prosperity_watch|fundamental_watch|risk_watch",
   "source_module": "M8_safety_margin",      # 新增：规则来源模块
   "trigger": "现价 ≤ 1500 元",               # 结构化触发器（数值阈值）
   "severity": "info|warn|critical",
   "action": "watch|alert|action",            # 新增：分层动作
   "message": "跌破买入区间，可分批建仓"}
  ```
